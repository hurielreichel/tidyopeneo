...
The **openeowrap** package was created with the intention to make the [openeo](https://cran.r-project.org/web/packages/openeo/index.html) package processes more flexible. This is due to the fact that the openeo processes are usually too "APIish". Therefore, so far, when using openeo processes, the feeling is mainly that you are not handling R code, but an specific API. For example, when trying a simple [filter_bands()](https://processes.openeo.org/#filter_bands), the code would look as something like this:
...
<img src="struc.png" alt="openEO processes and their relative wrapped functions in openeowrap" width="600"/>
...
From now on, we can work completely in a *tidy* syntax and use even the infamous pipes ( %>% ). The first example below demonstrates the filter_bands process and how it is wrapped in openeowrap. 
...
Here what also comes to hand is that you can use openEO API processes as usual on "datacube" classes from openeowrap. This is pretty useful, and the other way around is also possible, if the datacube() constructor is used to recreate a "datacube " class. 
...
Another interesting point of openeowrap is to have more examples of the multiple usages coming from the differentes API processes. This is still lacking in openEO documentation and it is believed this package could help. This is all been built. 

## resample

As stated, resample() is the single function in the actual stage of openeowrap that is not coming from dplyr, but it is a complete new function. It is ment as an example of a way to simplify different processes into one single wrapped function. 
...
r = function(data, context) { p$mean(data) }

dc_grouped <- dc %>% group_by(.reducer = r, .geometries = polygons)

```

## Compute Results

In order to demonstrate a long pipe with also exports, the following example is a good representation of the communication between openeo and openeowrap :
...
# compute_result(graph = graph, output_file = "openeowrap.png")
# png("openeowrap.png")
```

Here it is also clear how openeowrap functions always print all the underlying openeo processes, which is not only useful for debugging, but maintains a good transparency with openeo, making of it more user-friendly (I hope...).
